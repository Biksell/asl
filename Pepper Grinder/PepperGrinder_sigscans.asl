...
init {
    //"48 8D ?? 10 48 83 ?? 01 75 ?? 0F1F ?? ?? ???????? 8B 05 ???????? 83 ?? 01 74 ?? 48 8B ?? 8B ?? 48 8B ?? ???????? 48 8B ?? ?? 48  8B ?? 48 8B ?? ???????? 4C 89 ?? ???????? E8 ???????? 48 8B ?? ???????? 48 8B ?? E8 ???????? 48 8D ?? 10 48 83 ?? 0175 ?? 66 ???? ?? ?? ?? 8B 05 ???????? 83 E8 ??"
    IntPtr paused = vars.Helper.ScanRel(3, "48 8b ?? ???????? 45 8b ?? 49 ?? ?? 04 45 8b ?? 49 ?? ?? 49 ?? ?? 04 4d 8b ?? 49 8d ?? ?? 48 89 ?? ???????? E8 ???????? 44 ?? ?? ???????? 33 ?? 4c 8b ?? ???????? 44 8b ?? 45 8b ?? ?? 45 85 ?? 74 ?? F2");
    if (paused == IntPtr.Zero) {
        throw new Exception("Not found");
    }
    print(paused + "");

    vars.Helper["paused"] = vars.Helper.Make<bool>(paused, 0x2E0, 0x18, 0x120);
}
