...
init {
    vars.Helper.TryLoad = (Func<dynamic, bool>)(mono => {
        ...
        vars.Helper["sbEntries"] = mono.MakeList<IntPtr>("ScoreboardUpdater", "instance", "entries");
...
update {
    current.stars = ((List<IntPtr>)current.sbEntries).Select(address => {
        return vars.Helper.Read<int>(address + 0x40);
    }).ToList().Max();

    if(vars.doubleSplit.ElapsedMilliseconds > 1000) vars.doubleSplit.Reset();
...
split {
    return (settings["split_req"] && !vars.doubleSplit.IsRunning && current.stars == current.starRequirement && old.stars != current.stars) ||
